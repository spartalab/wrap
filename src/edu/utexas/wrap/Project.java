...
/** A high-level representation of a multi-market, multi-purpose travel demand model.
 * A Project is the top-level organizational unit in wrap, encapsulating all data and
 * configuration needed to run a complete four-step travel demand modeling process.
 * 
 * <p>This class defines:
 * <ul>
 *   <li>The set of {@link TravelSurveyZone}s for which demand is modeled</li>
 *   <li>The {@link Market} instances representing distinct traveler segments</li>
 *   <li>The {@link Assigner} instances that load demand onto the network</li>
 *   <li>An initial set of {@link NetworkSkim}s used for trip distribution and mode choice</li>
 *   <li>{@link SurrogatePurpose} instances that provide pre-computed demand profiles</li>
 * </ul>
 * 
 * <p>The Project is loaded from a properties file ({@code *.wrp}) located in a project
 * directory. This directory serves as the base path against which all relative file
 * references are resolved. The properties file specifies paths to zone data, skim files,
 * market definitions, and assigner configurations.
 * 
 * <p>A typical workflow is:
 * <ol>
 *   <li>Load the Project from a {@code .wrp} file</li>
 *   <li>Run each Market's trip generation, distribution, and mode choice steps</li>
 *   <li>Initialize Assigners with the resulting OD profiles</li>
 *   <li>Iterate the assignment to convergence</li>
 *   <li>Extract updated skims for feedback iterations</li>
 * </ol>
 * 
