...
public class TolledEnhancedLink extends TolledLink {
	...
	private Double signalDelay() {
		//	S(v) == s / max(0.1, 1 - v/s)
		return s/Math.max(0.1,
				1-(getFlow()/saturatedFlowRate));
	}
	...
	private Double r(Double x) {
		...
		return Math.sqrt(
				Math.pow(conicalParam,2)*Math.pow(g(x),2)
				+Math.pow(beta(),2)
				);
	}
	...
	private Double signalPrime() {
		...
		//		d( s/(1 - v/m) )/dv
		//		==	m*s / (m-v)^2
		if (vOverS<=0.875) return (saturatedFlowRate * s)/
				Math.pow((saturatedFlowRate-getFlow()),2);


		//	Else if u >= 0.925:
		//		d( s/0.1 )/dv == 0
		else if (vOverS>=0.925) return 0.0;


		//	Else: (i.e. 0.925 > u > 0.875)
		//		d( a*(v/m)^3 + b*(v/m)^2 + c*(v/m) + d )/dv
		//		==	((3 * a * v^2) + (2 * b * m * v) + (c * m^2)) / m^3
		else return (3*a*Math.pow(getFlow(),2)) + (2*b*saturatedFlowRate*getFlow()) +
	...
	private Double hIntegral() {
		...
		return getFlow()*(1-beta()) + rIntegral()
				- conicalParam*gIntegral();
	}
	...
	private Double rIntegral() {
		...
		Double smallRootTerm = Math.sqrt(
				Math.pow(a, 2) * Math.pow(h, 2) * Math.pow(c, 2)
				+ (Math.pow(b, 2) * Math.pow(c, 2))
				);

		// sqrt(	(a^2 * v^2) - (2 * c * h * a^2 * v) + (h^2 * a^2 * c^2) + (b^2 * c^2)  )
		Double bigRootTerm = Math.sqrt(
				Math.pow(a, 2) * Math.pow(v, 2)
				- (2.0*Math.pow(a, 2)*h*v*c)
				+ (Math.pow(a, 2)*Math.pow(h, 2)*Math.pow(c, 2))
				+ (Math.pow(b, 2)*Math.pow(c, 2))
				);
